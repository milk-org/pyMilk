...
@pytest.mark.parametrize('shape', [(3, ), (2, 3), (2, 3, 4), (3, 4, 5),
                                   (5, 6, 7)])
@pytest.mark.parametrize('location', [-1, 0])
def test_data_conservation(shape: tuple[int, ...], location: int):
...
def test_shared_nonshared_overlap(location1: int, location2: int):
    ...
    shm_shared_1 = SHM('shared_test', data_shared,
                       location=location1)  # Create
    shm_private = SHM('shared_test', data_private, location=location2,
                      shared=False)  # Create
